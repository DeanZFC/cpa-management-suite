...
- 管理页面可直接新增 CPA 认证账号：支持粘贴或选择 JSON 文件，并设置显示名称、代理、Base URL、优先级、备注、WebSocket 和 API 模式。
- 已有账号的“编辑”入口专注于修改并发容量；认证内容仍由新增账号流程提交给 CPA，保存后由 CPA 自动重新加载认证文件。
...
页面使用模块菜单布局，当前包含“账号管理”“模型价格”和“插件设置”。切换到“模型价格”后选择平台，点击“同步价格”即可立即拉取该平台的官方价格，不受自动刷新间隔限制；也可以直接编辑表格中的价格后保存。
...
首次安装插件时，CPA 仍需要启用插件目录；完成安装后，`default_capacity`、拒绝策略和价格同步配置可以直接在 CPAMP 的“插件管理 → 编辑配置”页面修改。CPA Management Key 请在插件自己的“插件设置”页面中填写，不需要再登录服务器编辑配置文件。
...
plugins:
  ...
  configs:
    cpa-management-suite:
      enabled: true
      cpa_management_key: "" # 旧版兼容项；新版本请在“插件设置”中填写
      priority: 100
...
管理页面在 CPA 管理页面的插件菜单中打开，或直接访问。打开“插件设置”后，在密码框中填写 CPA Management Key：

```text
/v0/resource/plugins/cpa-management-suite/dashboard
```

页面中的修改操作使用当前浏览器会话中的 CPA Management Key。密钥不会由插件注入 HTML，也不会写入插件状态文件；刷新页面仍会保留，关闭浏览器后需要重新填写。两套 CPA 使用各自的 `config.yaml`、插件状态文件和管理密钥，互不影响。

`cpa_management_key` 仍兼容旧版配置，但不再作为插件配置字段展示，也不会注入管理页面。插件设置页使用密码框和浏览器会话存储，用于浏览器调用 CPA 的管理接口；请只允许可信用户访问插件页面，不要提交到 GitHub 或公开截图。
...
项目名称已从早期版本的 `CPA Account Capacity`、`CPA Management Suite` 改为 `CPA-A Manager`。为了兼容已安装版本，插件 ID、管理接口和状态文件仍保留原有名称及 `account-capacity` 路径。
